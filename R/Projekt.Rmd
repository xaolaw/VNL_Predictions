...
logistic <- glm(Winner ~ . - Match_Date - Nationality -Player.Name -Position -Reception_Successful -Dig_Attempts -Set_Successes -Year -Player_ID -Reception_Attemps -Block_Rebounds, family = binomial, data = train)
summary(logistic)
```
...
```{r}
set.seed(123)
train <- sample(1:nrow(dataset), 0.7 * nrow(dataset))  # 70% training data
test <- setdiff(1:nrow(dataset), train)

rf <- randomForest(Winner ~ . - Match_Date -Player.Name -Player_ID, data = dataset, subset = train, importance = TRUE)

pred_rf <- predict(rf, newdata = dataset[test, ])

mse_rfPL <- mean((pred_rf - dataset$Winner[test])^2)
print(mse_rfPL)
```
### Stworzenie modelu randomForest dla zbioru Polskich zawodników
```{r}
trainPL <- sample(1:nrow(PolandSet), 0.7 * nrow(PolandSet))  # 70% training data
testPL <- setdiff(1:nrow(PolandSet), trainPL)

rfPL <- randomForest(Winner ~ . - Match_Date -Player.Name -Player_ID, data = PolandSet, subset = trainPL, importance = TRUE)

pred_rfPL <- predict(rfPL, newdata = PolandSet[testPL, ])
  
mse_rfPL <- mean((pred_rfPL - PolandSet$Winner[testPL])^2)
print(mse_rfPL)
```
#### Wyniki dla zbioru wszystkich siatkarzy
```{r}
predicted_rf_binary <- ifelse(pred_rf > 0.5, 1, 0)
conf_matrix <- table(Predicted = predicted_rf_binary, Actual = dataset$Winner[test])
...
conf_matrixPL <- table(Predicted = pred_rf_binaryPL, Actual = PolandSet$Winner[testPL])
