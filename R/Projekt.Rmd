...
```{r}
set.seed(123)
rf_pos_PL <- randomForest(Position ~ . - Match_Date -Player.Name -Player_ID -Versus -Winner, data = PolandSet, subset = trainPL_rf, importance = TRUE)

pred_rf_pos_PL <- predict(rf_pos_PL, newdata = PolandSet[testPL_rf, ])
conf_matrix_pos_PL <- table(Predicted = pred_rf_pos_PL, Actual = PolandSet$Position[testPL_rf])

print(conf_matrix_pos_PL)

accuracy_pos_PL <- sum(diag(conf_matrix_pos_PL)) / sum(conf_matrix_pos_PL)
print(paste("Accuracy:", round(accuracy_pos_PL, 4)))
```
