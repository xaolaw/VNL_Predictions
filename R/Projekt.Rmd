...
```
```{r}
pos_pred_boost <- predict(pos_boost, newdata = test, n.trees = 5000,type = "response")
class_names = colnames(pos_pred_boost)[apply(pos_pred_boost, 1, which.max)]
result = data.frame(test$Position, class_names)
 
print(result)
conf_mat = confusionMatrix(test$Position, as.factor(class_names))
print(conf_mat)
```
```{r}
summary(pos_boost)
